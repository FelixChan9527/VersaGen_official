...
class ControlNet(ControlNetModel):
    ...
    def forward(
        ...
        controlnet_cond: torch.FloatTensor,     # c
        ...
        # controlnet_cond大小 [b, 10, 3, h, w] 
        b, _, _, _, _ = controlnet_cond.shape
        controlnet_cond_emb = []
        for b_i in range(b):
            obj_idx = torch.nonzero(sample_idx[b_i] != -1, as_tuple=True)   # 获取有物体的tensor索引
            controlnet_cond_i = controlnet_cond[b_i][obj_idx]   # 按索引提取tensor

            assert len(controlnet_cond_i.shape) == 4
            controlnet_cond_i = controlnet_cond_i
            controlnet_cond_i = self.controlnet_cond_embedding(controlnet_cond_i)   # 图像编码，使之与噪声特征大小对应
            n, c, h, w = controlnet_cond_i.shape
            controlnet_cond_i = controlnet_cond_i.sum(dim=0)  # [c, h, w]   # 数目上特征进行取平均
            controlnet_cond_emb.append(controlnet_cond_i)
        
        controlnet_cond_emb = torch.stack(controlnet_cond_emb, dim=0)   # 拼成统一的特征

        # 我认为不建议用这一组，虽然运行速度会快，但是可能会导致模型对通道较后的物体
        ...
        # controlnet_cond_emb = controlnet_cond.sum(dim=1) / n  # [b, c, h, w]   # 数目上特征进行取平均

        # b 4 96, 96 [b, c]
        sample = sample + controlnet_cond_emb   # 直接相加 后期通过这个权重进行调整
